...
const UploadAccessPanel = ({ doc, onError, onSuccess }) => {
    ...
    const handleToggle = useCallback(() => {
        if (!user?.uid) {
            onError?.('请先登录再管理图片上传权限');
            return;
        }

        if (!accessState.token || accessState.ownerId !== user.uid) {
            onError?.('请先用会员兑换码激活当前账号的图片上传权限');
            return;
        }
        ...
        onSuccess?.(nextState.enabled ? '图片上传已开启' : '图片上传已关闭');
    }, [accessState, onError, onSuccess, updatePreferenceState, user?.uid]);

    const handleActivate = useCallback(async () => {
        if (!user) {
            onError?.('请先登录再激活图片上传权限');
            return;
        }
        ...
        try {
            ...
            updatePreferenceState(nextState);
            setMembershipKey('');
            onSuccess?.('兑换码验证成功，当前账号已开启图片上传');
        } catch (error) {
            onError?.(error.message || '激活失败');
        } finally {
            setIsActivating(false);
        }
    }, [membershipKey, onError, onSuccess, updatePreferenceState, user]);
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-3">
                            <div>
                                <div className="font-medium text-gray-900 dark:text-white">图片上传权限</div>
                                <div className="text-xs text-gray-400 mt-0.5">
                                    用会员兑换码激活当前账号的 R2 图片上传能力
                                </div>
                            </div>
                            <button
                                ...
                                title={accessState.enabled ? '关闭图片上传' : '开启图片上传'}
                            >
                                <span
                        ...
                        <div className="mt-4 flex gap-2">
                            <div className="relative flex-1">
                                ...
                                <input
                                    value={membershipKey}
                                    onChange={(event) => setMembershipKey(event.target.value.toUpperCase())}
                                    placeholder="输入会员兑换码"
                                    className="w-full pl-9 pr-3 py-3 rounded-2xl border border-gray-100 dark:border-gray-800 bg-white/80 dark:bg-gray-900/60 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 outline-none focus:border-amber-300 dark:focus:border-amber-700 transition-colors"
                                />
                            </div>
                            <button
                                onClick={handleActivate}
                                disabled={isActivating}
                                className="px-4 py-3 rounded-2xl bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-medium transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-60 flex items-center gap-2"
                            >
                                {isActivating ? <Loader2 size={16} className="animate-spin" /> : null}
                                激活
                            </button>
                        </div>

                        <div className="mt-3 flex items-center justify-between text-[11px] text-gray-400">
                            <span>兑换码只用于给当前 UID 签发上传令牌。</span>
